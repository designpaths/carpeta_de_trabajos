		...
		$('.in5-parallax-bg-img').each(function(i,el){
			...
			el.updateParallaxBG = function(sc){
				...
				if(el.noScale){
					...
					if (!offsetY) { offsetY = useZoomToScale ? b.y*(1-1/scale) : 0 }
					if (!offsetX) { offsetX = useZoomToScale ? b.x*(1-1/scale) : 0 }
					var transY = useZoomToScale ? -b.y/scale - offsetY : -b.y/scale, transX = useZoomToScale ? -b.x/scale - offsetX : -b.x/scale,
					width =window.innerWidth/scale, height = window.innerHeight/scale;
					$el.css('transform', 'translateY('+transY+'px) translateX('+transX+'px)');
